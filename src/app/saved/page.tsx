...
export default function SavedPage() {
    ...
    return (
        <main className="min-h-screen bg-background pb-safe md:pb-8 flex flex-col">
            ...
            {/* Page Header */}
            <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12 border-b border-white/5">
            ...
            {/* Saved Grid */}
            <div className="flex-grow w-full max-w-7xl mx-auto px-4 md:px-8 py-12">
                {savedItems.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
                        {savedItems.map((product) => (
                            <div key={product.id} className="cursor-pointer group active:scale-[0.98] transition-transform flex flex-col">
                                ...
                                <button className="mt-4 font-[family-name:var(--font-outfit)] text-[11px] font-medium tracking-widest uppercase text-background bg-warm-white py-2.5 rounded-none md:hover:bg-accent transition-colors">
                                    MOVE TO CART
                                </button>
                            </div>
                        ))}
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <h2 className="font-[family-name:var(--font-cormorant)] text-2xl text-warm-white font-medium mb-2">Your wishlist is empty</h2>
                        <p className="font-[family-name:var(--font-outfit)] text-[13px] text-foreground/60 mb-8 max-w-sm">Tap the heart icon on any item to save it for later.</p>
                        <Link href="/shop" className="font-[family-name:var(--font-outfit)] text-[11px] font-medium tracking-widest uppercase border border-warm-white/40 text-warm-white px-8 py-3 hover:bg-warm-white hover:text-background transition-colors">
                            EXPLORE SHOP
                        </Link>
                    </div>
                )}
